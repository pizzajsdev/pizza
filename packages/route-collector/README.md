...
Also supports `.mdx` and `.md` files for markdown pages, with an optional `mdxRendererFile` option to specify the route
file to use as the renderer.
...
You can use the `collectRoutes` function to collect the routes from the file system, and build your own route
configuration with any framework that supports programmatically-generated routes.
